...
public class projetoMeteorologico {
    ...
    public static boolean validarTemperatura(double temp){ //O método recebe uma temperatura para verificar se ela é válida

        if(temp >= -50 && temp <= 60){ //se as temperaturas declaradas estiverem dentro dessas 
           return true;                //opções elas entram dentro if
        }else{
            return false;
        }
    ...
    public static String classificarClima(double Tmedia, int Tumid){ //criado outro método
        if(Tmedia > 30 && Tumid > 75){ // se estiver entre essa medidas ela retorna, senão vai para próxima
            return"MUITO QUENTE";
